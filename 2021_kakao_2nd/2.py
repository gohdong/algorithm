...
random_grade = [i for i in range(4500,5400)]
...
while count < 595:
    ...
    for result in game_result:
        if result['taken'] <= 10:
            loser = {
                'id' : result['lose'],
                'grade' : user_info[result['lose']-1]['grade'] - 1 if user_info[result['lose']-1]['grade'] - 1 >0 else 0
            }
            winner = {
                'id' : result['win'],
                'grade' : user_info[result['win']-1]['grade'] + 1 if user_info[result['win']-1]['grade'] + 1 < 10000 else 9999
            }
